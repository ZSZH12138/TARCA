...
export const twoGpuSnapshot: RuntimeSnapshot = {
  run: { run_id: "run-a", graph_id: "graph-a", status: "ACTIVE", phase: "NEURAL_TRAIN", total_tasks: 74, completed_tasks: 30, running_tasks: 2, failed_tasks: 0, pending_tasks: 42, progress_fraction: 30 / 74, eta_seconds: null, eta_status: "CALIBRATING", created_at_utc: "2026-08-26T12:00:00Z" },
  ...
  resources: [
    { resource_id: "host", label: "主机", kind: "HOST", expected_cpu_cores: 8, actual_effective_busy_cores: 19.2, expected_memory_bytes: 64 * 1024 ** 3, actual_memory_bytes: 87 * 1024 ** 3, utilization_percent: 76, temperature_celsius: null, power_watts: null, active_processes: 2, sampled_at_utc: "2026-08-26T12:01:00Z" },
    ...[0, 1].map((gpu) => ({ resource_id: `gpu-${gpu}`, label: `GPU ${gpu}`, kind: "GPU" as const, expected_cpu_cores: 4, actual_effective_busy_cores: 0, expected_memory_bytes: 20 * 1024 ** 3, actual_memory_bytes: (17 + gpu) * 1024 ** 3, utilization_percent: 93 - gpu * 4, temperature_celsius: 71 + gpu, power_watts: 402 - gpu * 8, active_processes: 1, sampled_at_utc: "2026-08-26T12:01:00Z" })),
  ],

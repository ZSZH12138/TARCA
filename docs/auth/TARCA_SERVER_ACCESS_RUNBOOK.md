...
本节适用于用户另行授权的 Stage1B 或后续长时间实验，不扩展第 5 节定义的远程操作权限。

### 11.1 点火前

1. 读取本次批准的科学配置，不从上一次结果推断或临时改写参数；
2. 验证配置、代码、来源和容器环境哈希；
3. 运行有界硬件探针，读取实际 CPU、内存、GPU、显存和可用磁盘；
4. 调度器按实际容量生成并发计划，不写死卡型、卡数、核数或内存；
5. 若探针预测超出授权时间、显存/内存不足或会过量订阅，停止并向用户报告，禁止静默降配实验。

“吃满硬件”指在依赖图允许且不发生 OOM、交换抖动或过量订阅时，使用全部安全可调度资源。数据生成、聚合或依赖等待阶段出现暂时空闲是正常现象，不得为追求表面利用率并发运行会改变科学身份的额外任务。

点火前还必须用一次最小检查识别 SSH 落点：若能使用 Docker，则按宿主机部署；若没有 Docker、但镜像要求、GPU 和 Python/CUDA 环境已经满足，则视为已经进入目标容器，禁止安装或嵌套 Docker，改用项目提供的容器内直跑入口。两种路径必须保持同一代码、配置、来源、依赖锁和恢复契约；若两者都不满足，停止并报告固定失败类别。

### 11.2 独立运行

实验必须由容器内独立于 SSH 和 Codex 会话的受监督进程运行。Codex 完成点火并交给用户后，即使关闭 Codex 或断开 SSH，任务仍应继续。禁止让 Codex 用持续轮询代替进程监督器。

当独立进程、心跳和只读前端均确认稳定后，Codex 必须立即向用户返回监控链接并结束主动等待，不得等待实验完成；后续由用户通过前端监督，只有在用户再次明确要求时才介入。

实验进程不得依赖前端或 SSH 转发进程存活。只读前端应绑定远端回环地址，并通过独立的本地 SSH 转发交给用户；转发中断只影响查看，不得停止实验。临时密钥副本、代理脚本和临时配置应在连接建立后删除，长期密钥仍只从既有用户环境变量指向的位置读取。

### 11.3 检查点与续跑

- 神经训练按既定周期原子写入检查点；
- 任务数据库记录每个任务、attempt、artifact 和依赖状态；
- `TARCA_24H_RESET`：服务器会在第 24 小时断电并整机清空，运行不得只依赖容器层或 Docker 命名卷；
- `TARCA_RESET_MARGIN`：点火时的剩余租期必须大于硬件探针预计运行时间再加 1 小时，否则等待下一台新服务器；
- `TARCA_HOST_BIND_RECOVERY`：运行数据库、artifact 和检查点必须写入宿主机绑定目录；整机清空前停止容器写入，并把该目录及 SHA-256 清单回收到本地；
- 服务器断电或容器重启后不自动续跑；
- `TARCA_MANUAL_RESUME`：新服务器只在恢复同一绑定目录、代码、配置、来源和镜像身份并通过校验后，由用户明确授权手动 `resume`；
- 用户明确要求后执行 `resume`，只复用通过哈希和科学身份校验的已完成 artifact 与检查点；
- 配置、代码、来源或科学身份变化时拒绝续跑，必须建立新的受控运行。

### 11.4 只读前端

点火后必须把只读监控前端交给用户。前端至少显示：

- 总任务、已完成、运行中、等待、失败；
- 进程是否存活、最近心跳和失败类别；
- 调度器期望的 CPU 核、内存、GPU 与显存；
- 实际忙核、CPU、内存、每张 GPU 利用率与显存；
- 最近检查点和基于完成历史计算的 ETA；
- 资源不足、停滞、OOM、检查点过旧和 ETA 不可计算等告警。

前端不得在确认运行结束前显示中间 CRPS、胜率、门禁状态或其它会诱导改方案的科学结果。它只负责真实运行监督，不提供写操作、停止按钮或远程 shell。

### 11.5 人工操作边界

用户负责监督并决定是否要求诊断、停止或续跑。Codex只在用户明确授权时执行点火、诊断、精确停止、手动续跑和最终 artifact 回收；不得因资源暂时空闲自行扩大任务范围。最终回收必须包含完整结果、失败行、运行数据库、环境/硬件/精度收据、日志和检查点索引，并在本地校验哈希后再建议用户关闭服务器。
